/*
  InterfaceKipper - a libary for the MeijWorks interface
...
#include "InterfaceKipper.h"

// -----------
// Constructor
// -----------
InterfaceKipper::InterfaceKipper(LiquidCrystal_I2C * _lcd,
...
// --------------------------
void InterfaceKipper::updateScreen(boolean _rewrite){  
  int temp = 0;
  int temp2 = 0;
  ...
  if (_rewrite){
    ...
    // Regel 1
    lcd->write_buffer(L_A_STEER, 1);

    // Regel 2
    lcd->write_buffer(L_SETPOINT, 2);

    // Regel 3
    lcd->write_buffer(L_ANGLE, 3);
...
// ---------------------------
int InterfaceKipper::checkButtons(byte _delay1, byte _delay2){
  if (button1_flag){
...
// --------------------------------
void InterfaceKipper::calibrate(){
  // Temporary variables
  int _temp, _temp2, _temp3;
  
  // Steer calibration
  lcd->write_buffer(L_CAL_STEER, 0);
  ...
  while(true){
    if(checkButtons(0, 0) == -1){
      ...
      lcd->write_buffer(L_CAL_STEER_AD, 3);
      
      lcd->write_screen(-1);

      // Loop through calibration process
      for(int i = 0; i < 3; i++){
        _temp = implement->getSteerCalibrationPoint(i);
        _temp2 = _temp / 10;
  ...
  // ANGLE calibration
  lcd->write_buffer(L_CAL_ANGLE, 0);
  lcd->write_buffer(L_CAL_ACCEPT, 1);
  lcd->write_buffer(L_CAL_DECLINE, 2);
  lcd->write_buffer(L_BLANK, 3);
  ...
  while(true){
    ...
    if(checkButtons(0, 0) == -1){
      ...
      lcd->write_buffer(L_CAL_ENTER, 2);
      lcd->write_buffer(L_CAL_ANGLE_AD, 3);

      lcd->write_screen(-1);

      // Loop through calibration process
      for(int i = 0; i < 3; i++){
        _temp = implement->getAngleCalibrationPoint(i);
        _temp2 = _temp / 10;
